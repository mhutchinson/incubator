...
var (
	...
	vindexURL    = flag.String("vindex_url", "", "Alias for base_url.")
	...
	inLogOrigin  = flag.String("in_log_origin", "go.sum database tree", "The expected origin for the input log.")
	outLogPubKey = flag.String("out_log_pub_key", "", "The public key to use to verify the output log checkpoint.")
	outLogOrigin = flag.String("out_log_origin", "SumDBIndex", "The expected origin for the output log.")
	modRoot      = flag.String("mod_root", "", "The path to a go module checked out locally via git.")
	moduleFlag   = flag.String("module", "", "Alias for mod_root or module name.")
)
...
func run(ctx context.Context) error {
	if *baseURL == "" && *vindexURL != "" {
		*baseURL = *vindexURL
	}
	if *modRoot == "" && *moduleFlag != "" {
		*modRoot = *moduleFlag
	}
	if *modRoot == "" {
		return errors.New("mod_root or module flag must be provided")
	}

	// If modRoot is not a valid directory, check common git clone paths
	if s, err := os.Stat(*modRoot); err != nil || !s.IsDir() {
		candidates := []string{
			"/usr/local/google/home/mhutchinson/git/transparency-dev/tessera-project/main",
			"../tessera-project/main",
			"../../tessera-project/main",
		}
		for _, c := range candidates {
			if s2, err2 := os.Stat(c); err2 == nil && s2.IsDir() {
				*modRoot = c
				break
			}
		}
	...
	if *baseURL == "" {
		...
		cli, err := newClientFromFlags()
		if err != nil {
			return err
		}
		inLogCli, err := newInputLogClientFromFlags()
		if err != nil {
			return err
		}
		sumFetcher = func(ctx context.Context, modName string) (map[string]modData, error) {
			return queryIndex(ctx, cli, inLogCli, modName)
		}
	}
...
func queryIndex(ctx context.Context, cli *client.Client, inLogCli *client.InputLogClient, modName string) (map[string]modData, error) {
	resp, err := cli.LookupAllKey(ctx, modName, 10000)
	if err != nil {
		return nil, fmt.Errorf("failed to look up key: %w", err)
	}

	if !resp.Exists || len(resp.Indices) == 0 {
		return make(map[string]modData), nil
	}

	versions := make(map[string]modData, len(resp.Indices))

	klog.V(1).Infof("Dereferencing %d pointers", len(resp.Indices))
	for leaf, err := range inLogCli.Dereference(ctx, resp.RawInputLogCP, resp.Indices) {
		if err != nil {
			return nil, fmt.Errorf("failed to get leaf at index %d: %w", leaf.Index, err)
		}
		version, data, err := parseLeaf(leaf.Index, leaf.Data)
		if err != nil {
			return nil, fmt.Errorf("failed to parse leaf at index %d: %w", leaf.Index, err)
		}
...
func parseLeaf(idx uint64, data []byte) (string, modData, error) {
	...
	if len(lines) < 2 {
		return "", modData{}, fmt.Errorf("expected at least 2 lines but got %d", len(lines))
	}

	line0Parts := line0RE.FindStringSubmatch(lines[0])
	if len(line0Parts) < 4 {
		return "", modData{}, fmt.Errorf("invalid line 0 format: %q", lines[0])
	}
	line0Module, line0Version, zipHashB64 := line0Parts[1], line0Parts[2], line0Parts[3]

	line1Parts := line1RE.FindStringSubmatch(lines[1])
	if len(line1Parts) < 4 {
		return "", modData{}, fmt.Errorf("invalid line 1 format: %q", lines[1])
	}
...
func newClientFromFlags() (*client.Client, error) {
	if *baseURL == "" {
		return nil, errors.New("base_url flag must be provided")
	}
	var outV note.Verifier
	if *outLogPubKey != "" {
		var err error
		outV, err = note.NewVerifier(*outLogPubKey)
		if err != nil {
			return nil, fmt.Errorf("failed to construct output log verifier: %w", err)
		}
	}
	var inV note.Verifier
	if *inLogPubKey != "" {
		v, err := note.NewVerifier(*inLogPubKey)
		if err != nil {
			return nil, fmt.Errorf("failed to construct input log verifier: %w", err)
		}
		inV = v
	}
	origin := *inLogOrigin
	...
	outOrigin := *outLogOrigin
	if outOrigin == "" {
		if outV != nil {
			outOrigin = outV.Name()
		} else {
			outOrigin = "SumDBIndex"
		}
	}
	cfg := client.VerifierConfig{
		OutputLogOrigin:   outOrigin,
		OutputLogVerifier: outV,
		InputLogOrigin:    origin,
		InputLogVerifier:  inV,
	}
	return client.New(*baseURL, cfg, http.DefaultClient)
}

func newInputLogClientFromFlags() (*client.InputLogClient, error) {
	if *baseURL == "" {
		return nil, errors.New("base_url flag must be provided")
	}
	var inV note.Verifier
	...
	if err != nil {
		return nil, fmt.Errorf("failed to construct input log URL: %w", err)
	}
	return client.NewInputLogClient(u, origin, inV, http.DefaultClient)
}
