...
go run ./vindex/v1/cmd/sumdbindex \
...
The command above starts a web server that hosts the following URLs:
 - `/` - interactive Web UI for searching and inspecting the index
 - `/inputlog/` - the [tlog-tiles][] base URL for a proxy of the SumDB API
 - `/vindex/v1/lookup/<key-hash>` (or `/index/<key-hash>`) - the C2SP verifiable lookup API
 - `/checkpoint` - the signed Output Log checkpoint
...
The SumDB log is processed into a verifiable map which can be looked up using the client in `./vindex/v1/cmd/vindex`.
Below is an example of querying the index to list all releases of `github.com/transparency-dev/tessera`:

```shell
go run ./vindex/v1/cmd/vindex \
  --vindex_url=http://localhost:8088 \
  --out_log_pubkey="SumDBIndex+a5ed0e81+AXEnbaKj+9gCH3f69vcQokgkcFocCl+GlaMXrAg8mRzd" \
  --in_log_pubkey="sum.golang.org+033de0ae+Ac4zctda0e5eza+HJyk9SxEdh+s3Ux18htTTAD8OuAn8" \
  --in_log_origin="go.sum database tree" \
  --key="github.com/transparency-dev/tessera" \
  --dereference \
  --in_log_url="http://localhost:8088/inputlog/"
...
When querying large result sets or paging through historical entries, use `--before` to query before a specific input log index, or `--all` to traverse all pages:
...
Building the VIndex for the first time consumes and indexes all entries in the log:
```shell
...
Building the VIndex a second time reloads state from disk and verifies checkpoints:
```shell
time OUTPUT_LOG_PRIVATE_KEY=PRIVATE+KEY+SumDBIndex+a5ed0e81+AYT6tfHpqGaSoH0gYpM7fhj1tEkM3wwYR/IhtiYh1pnj \
  go run ./vindex/v1/cmd/sumdbindex \
  --storage_dir ~/vindex-sumdb/ \
...
### Benchmarking WASM MapFn

While `cmd/sumdbindex` runs the in-process pure Go mapper, the sandboxed WebAssembly implementation on Go SumDB is benchmarked via `vindexd`:

```shell
/usr/bin/time -v go run ./vindex/v1/cmd/vindexd \
  --mode=publisher \
  --input_log_url="file://$HOME/log-clones/sumdb/" \
  ...
  --output_log_dir=/tmp/vindex-wasm-sumdb/outlog \
  --db_path=/tmp/vindex-wasm-sumdb/db \
  --mpt_dir=/tmp/vindex-wasm-sumdb/mpt \
  --tile_cache_dir=/tmp/vindex-wasm-sumdb/tiles \
  --fetch_workers=4 \
  --oneshot
```

See [Benchmarks & Empirical Evaluation](../../docs/BENCHMARKS.md) for full telemetry comparing Pure Go, Single-Fetch WASM (`--fetch_workers=1`), and Parallel-Fetch WASM (`--fetch_workers=4`).

## Performance & Benchmarks

The V1 pipeline ingests directly from the live public Go SumDB (`sum.golang.org`) with parallel HTTP tile fetching, concurrent module mapping, and zero-WAL chunk commits.

### Live Remote Ingestion (60,965,405 Leaves)
...
- **On-Disk Footprint**:
  ...
  - Output Log (`outputlog`): **8 KB**
  - Pruned Tile Cache (`tilecache`): **28 KB** (managed by `TileReaper`)
  - **Total Disk Space**: **~420 MB**
- **Client Verification**: Verified against local git checkouts (`sumdbverify`) and standalone CLI lookups (`vindex`) with 100% cryptographic proof verification.

## Monitoring

The server exports metrics via Prometheus on the `/metrics` endpoint and standard Go pprof handlers on `/debug/pprof/`.

Key areas covered by the metrics include:
- **`vindex_map_duration_seconds`**: Time spent mapping leaf bundles.
- **`vindex_pebble_apply_duration_seconds`**: Duration of atomic batch commits to Pebble.
- **`vindex_indexing_lag`**: Difference between target Input Log size and current indexed size.
- **`vindex_lookup_latency_seconds`**: Latency of the ReadServer lookup endpoint.
