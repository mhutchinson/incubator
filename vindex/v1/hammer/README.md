...
### 3.1 Synthetic Leaf Generator (`vindex/hammer/generator.go`)
...
### 3.2 Tessera Sequencer (`vindex/hammer/sequencer.go`)
...
### 3.3 Drip-Feed Server (`vindex/hammer/server.go`)
...
- **Drip Schedules**:
  - **Steady Drip**: Releases 1 queued checkpoint every $\Delta t$ seconds (or at `--drip_rate` CP/sec).
...
### 3.4 Verifying Concurrent Readers (`vindex/hammer/reader.go`)

A pool of concurrent worker goroutines querying `vindexd` using `vindex/client.Client`:
...
- **Monotonicity Verification**:
  ...
  - Asserts that for any subsequent lookup at $S_{new} \ge S_{old}$, the returned index set $I_{new}$ is a superset of $I_{old}$ and the common prefix is identical ($I_{new}[:len(I_{old})] == I_{old}$).

### 3.5 Real-Time Metrics Analyzer (`vindex/hammer/analyzer.go`)
...
- **Invariant Assertions**:
  ...
  - `BoundsViolations`: Index $\ge$ `InputLogSize` or $< 0$.
...
| :--- | :--- | :--- |
| **Monotonicity** | $I_{t+1} \supseteq I_t$ for any key across advancing log sizes | Critical (Data loss or index rollback) |
| **Bounded Indices** | $\forall i \in I: 0 \le i < \text{InputLogSize}$ | Critical (Index leakage of uncommitted data) |
| **Mini-Log Equality** | $\text{CompactRange}(I).\text{Root}() == \text{MPT}.\text{Value}(\text{Key})$ | Critical (Cryptographic map tampering) |
| **Non-Inclusion** | Key $\notin \text{Log} \implies \text{Exists}=\text{false} \land \|I\|=0$ | Critical (False positive index response) |
...
```bash
# Build the hammer
go build -o bin/hammer ./vindex/cmd/hammer
