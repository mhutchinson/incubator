...
This benchmark measures full end-to-end ingestion, indexing, publication, and cryptographic verification of the entire public Go Module Sum Database (`sum.golang.org`) directly from the live remote log.
...
- **Dynamic Safe Watermark Pruning**: `TileReaper` actively purges cached tiles behind `SafeWatermark = mptDurableSize` (bounded by invariant `Target_CP >= Cached_Tiles >= m_kv_size >= Output_Size >= mptDurableSize`), maintaining a strictly bounded disk cache without risking crash-recovery replay durability.
