...
- **Prefix Search Extensibility**: By preserving canonical Claim Subject preimages on the host runtime, future extensions can construct auxiliary prefix-trie indices over raw subject strings without altering the core inverted chunk storage format or invalidating existing `'c'` chunk records.
