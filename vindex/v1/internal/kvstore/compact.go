...
// Clone returns a deep copy of the CompactRange with pre-allocated capacity for 64 levels.
func (cr *CompactRange) Clone() *CompactRange {
	if cr == nil {
		return nil
	}
	h := make([][sha256.Size]byte, len(cr.Hashes), 64)
	copy(h, cr.Hashes)
	return &CompactRange{
