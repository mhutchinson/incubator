...
### 3. Synchronous Commit Barrier & Serialized Batch Execution Loop
...
## 4. Startup Recovery & Ingestion Lifecycle
...
### Sub-Root Reconstruction Primitive (`store.GetSubRoot`)

During Stage 1 catchup when `exact == false || mptVersion < S_OUT`:
1. `RecoveryCoordinator` evaluates `MapFn` over the missing leaves in `[mptVersion .. S_OUT)` to identify modified key hashes.
2. For each key hash, it calls `store.GetSubRoot(keyHash, S_OUT)`:
   - Evaluates chunk records up to `S_OUT`, filtering out any in-flight entries where `absIdx >= S_OUT`.
   - Computes and returns `MiniLogRoot`.
3. Mutations are applied directly to the in-memory MPT with **zero storage writes or disk mutations**.
4. At the conclusion of Stage 1 replay, `mptMgr.Sync()` consolidates the MPT to disk, establishing `exact = true` and `mptDurableSize = S_OUT`. `TileReaper` then starts concurrently in Stage 3 with `SafeWatermark = mptDurableSize`.

### Ingestion Resume & Storage Idempotency Gap Absorption

When Stage 2 resumes live steady-state ingestion from `S_OUT` where a crash occurred after KV persistence but before Output Log publishing (`m_kv_size > S_OUT`), the Coordinator unconditionally invokes `store.WriteBatch` for batches starting at `S_OUT`. It relies on `store.WriteBatch`'s internal idempotency contract (`persistedKVSize`) to safely absorb the replay gap `[S_OUT .. m_kv_size)`:
...
- Guarantees zero duplicate storage mutations with no requirement for complex rollback or truncation logic.

### TileReaper Safe Watermark & Coordinator Wiring
...
- **Durability Guarantee**: Leaves below `mptDurableSize` are already committed to Pebble (`'c'` chunks) and durably fsync'd in MPT disk files (`exact == true`). Crash recovery never needs raw tiles below `mptDurableSize`.
- **Zero-Network Crash Recovery**: Retaining tiles in `[mptDurableSize .. m_kv_size)` allows Stage 1 startup recovery to replay MPT catchup up to `S_OUT` entirely from local cache in the event of an unclean crash (`exact == false` or lagging MPT).

---

## 5. Moving-Goalpost Prevention & Checkpoint Authentication Lifecycle
...
- **Initial State Inspection**: Logs discovered watermarks (`TargetSize`, `KVSize`, `OutputSize`, `MPTDurableSize`).
- **Recovery Mode Selection**: Explicitly logs decision (e.g., `"Selecting Instant Warm Start: exact is true and mptVersion equals OutputSize"` or `"Selecting Tile Replay Catchup: exact is false or mptVersion lags OutputSize, replaying leaves from X to Y"`).
...
- **Crash during Stage 1 (Startup Replay)**: Verify coordinator cleanly restarts and re-executes replay without database corruption.
- **Crash during Stage 2 (KV Committed, Output Log not yet written)**: `m_kv_size > S_OUT`. Because `store.WriteBatch` completes and syncs to disk before `publisher.PublishBatch` begins, storage state leads the Output Log. On restart, Stage 1 sets serving state to `S_OUT`, reads only valid chunks up to `S_OUT` via `store.GetSubRoot`, and Stage 2 resumes ingestion starting at `S_OUT`, relying on `store.WriteBatch`'s idempotency contract to safely absorb `[S_OUT .. m_kv_size)` without duplicate chunk mutations.
- **Crash during Stage 2 (Output Log written, MPT in-memory committed, but MPT not yet synced to disk)**: `S_OUT > mptVersion` or `exact == false`. On post-crash startup, `torchwood/mpt` reports `exact == false` because the file header was dirtied on the first `Set()` prior to a subsequent `Sync()`. Verify that on restart, Stage 1 detects `exact == false || mptVersion < S_OUT`, executes fast replay `[mptVersion .. S_OUT)`, recovers MPT in RAM, asserts root consistency, consolidates disk state via `mptMgr.Sync()`, and opens the read server in < 500ms.
...
- **Crash during Stage 3 (MPT Compaction / Tile Reaper)**: Verify atomic file renaming (`os.Rename`) prevents partial snapshot files from corrupting previous valid snapshots.
