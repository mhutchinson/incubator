				...
				if bm, ok := workerMapper.(BundleMapper); ok {
					...
					if err != nil {
						...
						for offset, entries := range bundleResults {
							...
							// Performance fast-path (NOT a functional requirement): check if guest entries
							// are already strictly sorted and unique. If true, bypass allocating a separate
							// keys slice and running slices.SortFunc/slices.Compact. Fallback sorts and deduplicates.
							isSortedUnique := true
